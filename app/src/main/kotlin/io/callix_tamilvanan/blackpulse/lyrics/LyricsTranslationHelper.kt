...
import io.callix_tamilvanan.blackpulse.api.DeepLService
import io.callix_tamilvanan.blackpulse.api.OpenRouterService
import io.callix_tamilvanan.blackpulse.api.OpenRouterStreamingService
            ...
            scope.launch(Dispatchers.IO) {
                try {
                    ...
                    if (effectiveApiKey.isBlank()) {
                        _status.value = TranslationStatus.Error(context.getString(io.callix_tamilvanan.blackpulse.R.string.ai_error_api_key_required))
                        return@launch
                    }

                    if (lyrics.isEmpty()) {
                        _status.value = TranslationStatus.Error(context.getString(io.callix_tamilvanan.blackpulse.R.string.ai_error_no_lyrics))
                        return@launch
                    }
                    ...
                    if (nonEmptyEntries.isEmpty()) {
                        _status.value = TranslationStatus.Error(context.getString(io.callix_tamilvanan.blackpulse.R.string.ai_error_lyrics_empty))
                        return@launch
                    }
                    ...
                    if (targetLanguage.isBlank()) {
                        _status.value = TranslationStatus.Error(context.getString(io.callix_tamilvanan.blackpulse.R.string.ai_error_language_required))
                        return@launch
                    }
                    ...
                    result
                        ...
                        .onFailure { error ->
                            ...
                            val errorMessage = error.message ?: context.getString(io.callix_tamilvanan.blackpulse.R.string.ai_error_unknown)
                    ...
                    if (e !is kotlinx.coroutines.CancellationException && isCompositionActive) {
                        val errorMessage = e.message ?: context.getString(io.callix_tamilvanan.blackpulse.R.string.ai_error_translation_failed)
                        _status.value = TranslationStatus.Error(errorMessage)
                    }
                }
            }
